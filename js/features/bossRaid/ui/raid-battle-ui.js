...
class RaidBattleUI {
    ...
    static getSvgIcon(name, extraClass = '') {
        const icons = {
            ...
            crosshair: `<svg class="raid-svg-icon ${extraClass}" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><line x1="12" y1="2" x2="12" y2="6"/><line x1="12" y1="18" x2="12" y2="22"/><line x1="2" y1="12" x2="6" y2="12"/><line x1="18" y1="12" x2="22" y2="12"/></svg>`,
            ...
            lock: `<svg class="raid-svg-icon ${extraClass}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`
        };
        return icons[name] || '';
    }
        ...
        allAvatars.forEach(av => {
            ...
            item.innerHTML = `
                <div style="position:relative;">
                    ...
                    ${!isUnlocked ? `<div class="avatar-lock-overlay" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.45);color:#ef4444;border-radius:6px;">${RaidBattleUI.getSvgIcon('lock')}</div>` : ''}
                </div>
...
// Constrói a lista visual de fases
                ...
                <div class="battle-main-layout">
                    ...
                    <div class="battle-left-column">
                        ...
                        <div class="boss-raid-arena" id="boss-raid-arena">
                            ...
                            <!-- 3. Camada Inferior: Palco dos Heróis -->
                            <div class="party-battle-row" id="party-battle-row">
                                ${players.map(p => {
                                    ...
                                    // Resolve raridade para cor temática do card (igual ao inventário)
                                    const avData = (typeof skillsData !== 'undefined' && skillsData[avId]) || (window.skillsData && window.skillsData[avId]) || {};
                                    const rarityKey = avData.rarity || 'COMMON';
                                    const rarityColors = {
                                        'COMMON': '#94a3b8',
                                        'RARE': '#38bdf8',
                                        'EPIC': '#c084fc',
                                    ...
                                    const cardColor = rarityColors[rarityKey] || '#38bdf8';

                                    return `
                                        <div class="hero-battle-card ${isSelf && isPartyPhase && !isDown ? 'active-turn' : ''} ${isDown ? 'is-downed' : ''} ${isHeroTargeted ? 'is-targeted' : ''}" id="hero-card-${p.uid}" style="--rarity-color: ${cardColor};">
                                            <div class="hero-pedestal"></div>
                                            <div class="hero-card-inner">
                                                <!-- Efeitos Holográficos idênticos ao inventário -->
                                                <div class="tcg-card-holo-frame"></div>
                                                <div class="hero-card-rarity-bar"></div>
                                                <div class="hero-card-img-wrap">
                                                    <div class="tcg-geo-pattern"></div>
                                                    <div class="tcg-foil-sparkles"></div>
                                                    <img src="${avatarSrc}" alt="" class="hero-battle-ghost-aura" aria-hidden="true" onerror="this.style.display='none'" />
                                                    <img src="${avatarSrc}" alt="${p.displayName || 'Herói'}" class="hero-battle-avatar-img" onerror="this.style.opacity='0.4'" />
                                                    ...
                                                    ${!isDown && ((isPartyPhase && playerHasActed) || (isBossPhase && isHeroTargeted && playerHasReacted)) ? `<div class="acted-check-badge">✓</div>` : ''}
                                                </div>
                                                <!-- Corpo: Apenas Nome, Subclasse e HP -->
                                                <div class="hero-card-body">
                                                    <div class="hero-name-label" title="${p.displayName || 'Codemancer'}">${p.displayName || 'Codemancer'}${isSelf ? ' (Você)' : ''}</div>
                                                    <div class="hero-subclass-label" title="${p.subclass || 'Aprendiz'}">${(p.subclass || 'Aprendiz').toUpperCase()}</div>
                                                    <div class="hero-hp-bar-container" title="Pontos de Vida: ${p.currentHp || 0} / ${p.maxHp || 600}">
                        ...
                        <div class="battle-action-dock" id="battle-action-dock" data-dock-key="${isPartyPhase ? (hasActed ? 'party_acted' : 'party_ready') : (isBossPhase ? (isTargeted ? (hasReacted ? 'boss_reacted' : 'boss_ready') : 'boss_idle') : 'other')}">
                            ${isPartyPhase && isAlive && !hasActed ? `
                                <div class="action-buttons-group">
                                    ...
                                    ${hasDownedPlayers ? `
                                        <button class="glow-button accent raid-action-btn" id="btn-action-revive" title="Reviver Aliado Caído (+40% HP)" style="background:rgba(239,68,68,0.25);border-color:#ef4444;color:#fee2e2;">
                                            <span class="btn-text">${RaidBattleUI.getSvgIcon('lightning')} REVIVER ALIADO</span>
                                        </button>
                                    ` : ''}
                                </div>
                    ...
                    <!-- COLUNA DIREITA: WORKSPACE COM DESAFIO E SAÍDA NO TOPO, IDE E GRIMÓRIO ABAIXO -->
                    <div class="battle-right-column">
                        <div class="battle-workspace-container">
                            <!-- 1. TOPO: Painel de Desafio e Saída Esperada Lado a Lado -->
                            <div class="battle-top-challenge-area">
                                <!-- Box Esquerdo: Desafio e Instruções -->
                                <div class="battle-problem-panel" id="challenge-modal-left">
                                    <div class="problem-section" id="raid-problem-section">
                                        ...
                                        <div class="story-block">
                                            <div class="character-block-body" id="raid-challenge-instruction">
                                ...
                                <!-- Box Direito: Saída Esperada dos Casos de Teste -->
                                <div class="battle-expected-output-panel">
                                    <div class="expected-output-box" id="raid-expected-output-box">
                                        <div class="expected-placeholder">
                                            <div class="expected-placeholder-header">
                                                <strong class="expected-title">SAÍDA ESPERADA</strong>
                                                <span class="expected-mode-badge">MESMA LINHA</span>
                                            </div>
                                            <div class="expected-placeholder-text">Aguardando seleção de ação para exibir os testes esperados.</div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <!-- 2. BAIXO: Painel do Editor C/C# (IDE) -->
                            <div class="battle-editor-panel ${!this.activeChallenge ? 'is-disabled' : ''}" id="battle-editor-panel">
                                <div class="editor-disabled-overlay" id="editor-disabled-overlay">
                                    ${RaidBattleUI.getSvgIcon('lock')}
                                    <span>ESCOLHA UMA AÇÃO NO PAINEL PARA DESBLOQUEAR A IDE</span>
                                </div>
                                <div class="battle-editor-header">
                                    <div class="editor-tabs">
                                        <span class="editor-tab active" id="raid-editor-tab-file">
                                            ${isCSharp ? 'PlayerAction.cs' : 'main.c'}
                                        </span>
                                    </div>
                                    <div class="editor-actions">
                                        <button id="btn-raid-editor-format" class="editor-btn" title="Formatar e Indentar Código" onclick="if(typeof app !== 'undefined' && app.ui && app.ui.formatCurrentEditor) app.ui.formatCurrentEditor();">
                                            Formatar
                                        </button>
                                ...
                                <div class="battle-editor-wrapper">
                                    ...
                                    <aside class="battle-notepad-drawer hidden" id="raid-modal-notepad-drawer">
                                        <div class="activity-drawer-header">
                                            ...
                                            <button class="activity-drawer-close" id="btn-close-raid-modal-notepad" title="Fechar Grimório">
            ...
            if (tests.length > 0) {
                ...
                expectedBox.innerHTML = `
                    <div class="expected-output-card">
                        <div class="expected-output-card-header">
                            <strong class="expected-title">SAÍDA ESPERADA</strong>
                            <span class="expected-mode-badge">
                                ${isSingleLine ? 'MESMA LINHA' : 'QUEBRAS (\\n)'}
                            </span>
                        </div>
                        <div class="expected-tests-list">${testListHtml}</div>
                    </div>
                `;
            } else {
                expectedBox.innerHTML = `
                    <div class="expected-output-card">
                        <div class="expected-output-card-header">
                            <strong class="expected-title">SAÍDA ESPERADA</strong>
                            <span class="expected-mode-badge">STATUS</span>
                        </div>
                        <div class="expected-tests-list">
                            <div class="expected-test-item" style="font-size:0.75rem;color:#94a3b8;font-style:italic;">Nenhuma saída de texto exigida. Retorno esperado: 0.</div>
                        </div>
                    </div>
                `;
            }
    ...
    renderVictoryScreen(raidData, boss, onClaimRewards) {
        ...
        if (btnClaim) {
            btnClaim.onclick = () => onClaimRewards(finalXp, finalTokens, boss);
        }
    }
