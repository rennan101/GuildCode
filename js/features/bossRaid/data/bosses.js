...
class BossDataManager {
     ...
     * Conforme Seção 18:
     * bossMaxHp = boss.baseHp * (1 + (partySize - 1) * 0.65) * levelScaling
     */
    static calculateBossStats(boss, partySize = 1, averagePartyLevel = 5) {
        ...
        const partyMultiplier = 1 + (safeSize - 1) * 0.65;
        const levelScaling = 1 + Math.max(0, (safeLvl - boss.recommendedLevel) * 0.05);

        const scaledHp = Math.round(boss.baseHp * partyMultiplier * levelScaling);
        const scaledAtk = Math.round(boss.baseAttack * (1 + (safeSize - 1) * 0.15) * levelScaling);
        const scaledDef = Math.round(boss.baseDefense * (1 + (safeSize - 1) * 0.10));
