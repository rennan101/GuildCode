    ...
    3: [
        ...
        { id: "sq3_5", title: "Combinar Funções", difficulty: "medium", xp: 25,
          ...
          validator: function(code, output) {
              ...
              if (!output.includes("70")) errors.push("Lyra ATK deve ser 70");
...
function generateSideQuests() {
    const templates = {
        ...
        8: [
            ...
            { id: "sq8_3", title: "Contar Comparações", difficulty: "easy", xp: 15, description: "Implemente busca binária contando quantas comparações foram feitas.", starterCode: '#include <stdio.h>\n\nint main() {\n    int vet[7] = {10,20,30,40,50,60,70};\n    int alvo = 40;\n    int ini = 0, fim = 6, comps = 0;\n    // Busca binaria contando comparacoes\n    printf("Comparacoes: %d\\n", comps);\n    return 0;\n}', hints: [{level:"I",text:"Incremente comps a cada comparação."},{level:"II",text:"comps++; antes de cada if."},{level:"III",text:"while (ini <= fim) {\n    int meio = (ini + fim) / 2;\n    comps++;\n    if (vet[meio] == alvo) break;\n    else if (vet[meio] < alvo) ini = meio + 1;\n    else fim = meio - 1;\n}"}], tests: [{input:"",expected:"Comparacoes: 2",description:"2 comparações"}], validator: function(code,output){let e=[];if(!output.includes("Comparacoes:"))e.push("Deve imprimir comparações");return{pass:e.length===0,errors:e};}},
            { id: "sq8_4", title: "Remover por Busca Binária", difficulty: "medium", xp: 25, description: "Encontre 30 por busca binária e remova, deslocando.", starterCode: '#include <stdio.h>\n\nint main() {\n    int vet[7] = {10,20,30,40,50,60,70};\n    int tam = 7;\n    // Busque 30 e remova\n    for (int i = 0; i < tam; i++) printf("%d ", vet[i]);\n    return 0;\n}', hints: [{level:"I",text:"Primeiro encontre a pos com busca binária, depois desloque."},{level:"II",text:"for (int i=pos;i<tam-1;i++) vet[i]=vet[i+1]; tam--;"},{level:"III",text:"// Após encontrar pos:\nfor (int i = pos; i < tam - 1; i++) vet[i] = vet[i+1];\ntam--;"}], tests: [{input:"",expected:"10 20 40 50 60 70",description:"30 removido"}], validator: function(code,output){let e=[];if(output.includes("30"))e.push("30 nao deve aparecer");if(!output.includes("40"))e.push("40 deve aparecer");return{pass:e.length===0,errors:e};}},
